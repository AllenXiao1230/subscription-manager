<script setup>
// (Script 內容完全沒有變動，請保留您現有的 <script setup> 區塊)
...
<style>
/* --- (*** 2. (REQUEST) 全新的「專業深色」CSS ***) --- */

/* 1. 全域與字體設定 */
:root {
  --color-bg: #18181B; /* 更深的背景 */
  --color-card: #27272A; /* 卡片/Modal 背景 */
  --color-border: #3F3F46; /* 邊框 */
  --color-text-primary: #E4E4E7; /* 主要文字 (淺灰) */
  --color-text-secondary: #A1A1AA; /* 次要文字 (稍暗) */
  --color-accent: #14B8A6; /* 亮點色 (青色) */
  --color-accent-hover: #0D9488;
  --color-danger-bg: #450A0A; /* 紅色按鈕背景 */
  --color-danger-text: #F87171; /* 紅色文字 */
...
input, select {
  ...
  width: 100%;
  box-sizing: border-box;
  transition: border-color 0.2s, box-shadow 0.2s; /* 加入動畫 */
}
input:focus, select:focus {
  outline: none;
  border-color: var(--color-accent);
  box-shadow: 0 0 0 3px rgba(20, 184, 166, 0.3); /* 焦點光暈 */
}
...
button {
  ...
  cursor: pointer;
  font-weight: 600;
  transition: background-color 0.2s; /* 加入動畫 */
}
button:hover {
  background-color: var(--color-accent-hover);
}
button.submit-btn {
  padding: 12px 16px; /* 讓主要按鈕更大 */
}
...
.payment-grid tbody tr:nth-child(odd) td {
  background-color: var(--color-card); /* 奇數行 (基礎) */
}
.payment-grid tbody tr:nth-child(even) td {
  background-color: #313134; /* 偶數行 (深色條紋) */
}
.status-cell:hover { 
...
.modal-overlay {
  ...
  backdrop-filter: blur(4px); /* (NEW) 毛玻璃效果 */
...
.modal-actions button[type="button"] { /* 取消按鈕 */
